...
public class BoardLogic : IBoardLogic
{
    ...
    public void RevealCell(BoardModel board, int row, int column)
    {
        ...
        // Bombs and numbered safe cells reveal only themselves. A zero-value
        // safe cell starts the recursive flood-fill operation.
        if (cell.IsBomb || cell.NumberOfBombNeighbors > 0)
        {
            VisitCell(board, cell);
            return;
        }
    ...
    /// <summary>
    /// Recursively reveals a connected region of zero-neighbor cells and the
    /// numbered boundary cells surrounding that region.
    /// </summary>
    public void FloodFill(BoardModel board, int row, int column)
    {
        ValidateBoard(board);

        // Base case 1: coordinates outside the board stop this branch.
        if (!IsValidCell(board, row, column))
        {
            return;
        }
        ...
        // Base case 2: do not revisit cells, reveal flagged cells, or reveal bombs.
        ...
        // Base case 3: numbered cells form the visible boundary of the fill.
    ...
    private static void VisitCell(BoardModel board, CellModel cell)
    {
        cell.IsVisited = true;
